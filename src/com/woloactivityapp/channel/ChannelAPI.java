...
public class ChannelAPI {
	 ...
	 * 
	 * @param URL
	 *            - Server Location - http://localhost:8888
	 * @param channelKey
	 *            - Unique Identifier for channel groups, server uses this to
	 *            push data to clients, can have multiple clients on the same
	 *            key, but only 1 channel per client
	 * @param channelService
	 *            - An Implementation of the ChannelService class, this is where
	 *            the function methods will get called when the server pushes
	 *            data
	 * @throws IOException
	 *             JSON Related
	 ...
	 *             Connection Related
	 */
	public ChannelAPI(String URL, String channelKey,
			ChannelService channelService) throws IOException,
	 ...
	 *            - Server Location - http://localhost:8888
	 * @param token
	 *            - Unique token returned by the App-Engine server
	...
	public void joinChannel(String URL, String token,
		...
		this.applicationKey = this.channelId.substring(this.channelId
				.lastIndexOf("-") + 1);
		if (channelListener != null) {
			this.channelListener = channelService;
		}
	...
	private void initialize() throws ChannelException {
		...
		String initUri = this.PROD_TALK_URL + "d?"
		...
		try {
			...
			if (resp.getStatusLine().getStatusCode() > 299) {
				throw new ChannelException("Initialize failed: "
						+ resp.getStatusLine());
			}
			...
			if (m.find()) {
				...
				for (int i = 0; i < 7; i++) {
					if (!m.find()) {
						throw new ChannelException("Expected iteration #" + i
								+ " to find something.");
					}
					if (i == 2) {
	...
	private void fetchSid() throws ChannelException {
		...
		try {
			...
			List<TalkMessage.TalkMessageEntry> entries = entry
					.getMessageValue().getEntries();
			if (!entries.get(0).getStringValue().equals("c")) {
				throw new InvalidMessageException(
						"Expected first value to be 'c', found: "
	...
	private void connect() throws ChannelException {
		String uri = getBindString(
				new BasicNameValuePair("AID", Long.toString(this.messageId)),
	...
	private String getBindString(NameValuePair... extraParams) {
		...
		params.add(new BasicNameValuePair("zx", RandomStringUtils.random(12,
				true, false)));
		...
		params.add(new BasicNameValuePair("RID", Integer
				.toString(this.requestId)));
		this.requestId++;

		return this.PROD_TALK_URL + "dch/bind?VER=8&"
				+ URLEncodedUtils.format(params, "UTF-8");
	}
	...
	private void longPoll() {
		...
		this.thPoll = new Thread(new Runnable() {
			private TalkMessageParser repoll() {
				String bindString = getBindString(
						new BasicNameValuePair("CI", "0"),
		...
		private String readSubmission() throws ChannelException {
			try {
				...
				throw new ChannelException(
						"Submission was not in expected format.", e);
			}
		}
	}
	...
	private static class TalkMessage {
		public enum MessageEntryKind {
			ME_STRING, ME_NUMBER, ME_EMPTY, ME_TALKMESSAGE
		}
		...
		private static ArrayList<TalkMessageEntry> parseMessage(
				BufferedReader reader) throws InvalidMessageException,
				IOException {
			...
			while (ch != ']') {
				...
				if (ch == '[') {
					...
					entries.add(new TalkMessageEntry(
							MessageEntryKind.ME_TALKMESSAGE, new TalkMessage(
									childEntries)));
				} else if (ch == '\"' || ch == '\'') {
					String stringValue = parseStringValue(reader, (char) ch);
					entries.add(new TalkMessageEntry(
							MessageEntryKind.ME_STRING, stringValue));
				} else if (ch == ',') {
					// blank entry
					entries.add(new TalkMessageEntry(MessageEntryKind.ME_EMPTY,
							null));
				} else {
					// we assume it's a number
					long numValue = parseNumberValue(reader, (char) ch);
					entries.add(new TalkMessageEntry(
							MessageEntryKind.ME_NUMBER, numValue));
				}
				...
				if (ch != ',' && ch != ']') {
					throw new InvalidMessageException("Expected , or ], found "
							+ ((char) ch));
				} else if (ch == ',') {
					ch = skipWhitespace(reader);
				}
		...
		private static String parseStringValue(BufferedReader reader, char quote)
				throws IOException {
			String str = "";
			for (int ch = reader.read(); ch > 0 && ch != quote; ch = reader
					.read()) {
				if (ch == '\\') {
					ch = reader.read();
					if (ch < 0) {
		...
		private static long parseNumberValue(BufferedReader reader,
				char firstChar) throws IOException {
			String str = "";
			for (int ch = firstChar; ch > 0 && Character.isDigit(ch); ch = reader
					.read()) {
				str += (char) ch;
				reader.mark(1);
			}
		...
		private static int skipWhitespace(BufferedReader reader)
				throws IOException {
			int ch = reader.read();
			while (ch >= 0) {
		...
		public static class TalkMessageEntry {
			...
			public String getStringValue() throws InvalidMessageException {
				if (mKind == MessageEntryKind.ME_STRING) {
					...
					throw new InvalidMessageException(
							"String value expected, found: " + mKind + " ("
									+ mValue + ")");
				}
			}
			...
			public long getNumberValue() throws InvalidMessageException {
				if (mKind == MessageEntryKind.ME_NUMBER) {
					...
					throw new InvalidMessageException(
							"Number value expected, found: " + mKind + " ("
									+ mValue + ")");
				}
			}
			...
			public TalkMessage getMessageValue() throws InvalidMessageException {
				if (mKind == MessageEntryKind.ME_TALKMESSAGE) {
					...
					throw new InvalidMessageException(
							"TalkMessage value expected, found: " + mKind
									+ " (" + mValue + ")");
				}
			}
	...
	private String getUrl(String command) throws IOException {
		String url = BASE_URL + CHANNEL_URL + "dev?command=" + command
				+ "&channel=";
	...
	/**
	 * If you successfully connect to the Channel, start pulling data. If you
	 * fail to connect, the error is printed to the terminal and the connection
	 * is closed.
	 * 
	 * @param xhr
	 */
	 ...
	 * @param message
	 * @param urlPattern
	 *            - where the server should look for the message. ex: "/chat"
	 * @return true
	 * @throws IOException
	 */
